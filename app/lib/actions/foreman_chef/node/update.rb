      ...
      class Update < Actions::EntryAction
        ...
        def plan(host, proxy)
          ...
          if node_exists_in_chef
            # we can't use host.run_list.list_changed? since it's after_save already
            if host.run_list_differs?
              plan_self :chef_proxy_id => proxy.id, :host_id => host.id
            else
              Rails.logger.debug "Run list does not differ to Chef server version, skipping update"
            end
          else
        ...
        def run
          ...
          proxy.update_node(host.name, host.run_list.as_chef_json)
        end
